...
- 当前事实：产品骨架、`SKILL.md`、标准 Prompt、执行脚本、eval 配置和 donor 适配层已落盘；当前正在重构“小红书登录判别 + 一次性登录决策 + 无登录风险披露 + final broadcast 边界”
...
- 若用户在无登录执行后再次明确要求登录，不得续跑旧 run；必须对同一链接组重新执行任务并生成新的 run。
