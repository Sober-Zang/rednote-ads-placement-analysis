...
- 任务分型只允许三类：标准任务；非标准任务但包含小红书链接；非标准任务且不包含小红书链接
- `prompt/used_prompt.md` 必须存在
- 标准任务：`prompt/used_prompt.md` = `assets/standard_analysis_prompt.md`
- 非标准任务但包含小红书链接：`prompt/used_prompt.md` = 用户本轮完整新输入
- 非标准任务且不包含小红书链接：不建立 run，不执行下载，不调用本 skill 主链
