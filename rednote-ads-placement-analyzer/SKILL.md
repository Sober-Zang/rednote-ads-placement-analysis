...
- 标准任务只有在单篇报告、综合报告与最终播报都完成后，才算任务完成；不要把“仅完成下载和归档”误判成任务结束。
...
开始执行前，优先阅读并遵循本文件；若需要定位 skill 本体，按以下顺序查找：

| 平台 | 第一优先级：工作区本地 skill 根 | 第二优先级：用户级全局 skill 根 |
| --- | --- | --- |
| macOS / Linux | `<workspace>/.agent/skills/rednote-ads-placement-analysis/` | `~/.agent/skills/rednote-ads-placement-analysis/` |
| Windows | `<workspace>\\.agent\\skills\\rednote-ads-placement-analysis\\` | `%USERPROFILE%\\.agent\\skills\\rednote-ads-placement-analysis\\` |

如果用户已经明确提供工作区，应优先使用工作区本地 skill 根；只有用户只表达“安装 skill”而没有指定工作区时，才退回用户级全局 skill 根。
...
先执行 `check-env`。如果环境已经满足，就直接继续，不要重复准备；只有 `check-env` 未通过时，才按下面这一套唯一安装方式补齐环境。
...
目标：从用户自然语言中提取候选链接，识别有效小红书链接，并初始化官方 run，为后续抓取、单篇报告与综合报告生成建立唯一合法入口。这一阶段只是报告交付的起点，不是任务终点。
...
- 标准任务一旦建立 run，就默认承诺后续要继续走到报告产出与最终播报；不要把本阶段完成误判成任务已经完成
...
- 若已经登录，直接继续后续任务
- 若尚未登录，则先自动清空失效的 `xhs_user_data_dir`，再在当前已打开的页面内直接进入 300 秒等待用户登录，不要先关闭再重新打开页面
...
- 若超时或浏览器被关闭，则回落为无登录模式继续执行；不要因为未登录就停止任务
...
目标：在标准任务下，必须使用标准提示词驱动单篇报告与综合报告生成；在自定义任务下，才切换到 run 专属 Prompt 解决用户指定目标。
