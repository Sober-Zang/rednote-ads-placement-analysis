...
- 分析前必须确认正文图片、关键评论图片等确实已经以可供模型读取和思考的方式提供给模型；若图片未被成功提供，不得假装“已经看过图片”
...
- 必须覆盖：
  ...
  - 风险边界
- 所有关键结论带引用标签
- 对关键主结论和关键子结论，不要只给结论，要补必要的例证；例证优先使用原文原句、原评论原句或图片中的明确信息
- 每一个子结构都要留出一个简短的自由发挥空间，确保模型在完成必要内容之外，还能补充最有价值的观察，但不要造成冗余
- 图片分析必须基于模型真实看到的图片内容，尤其是关键正文图片；若某张图未被成功提供给模型，应明确收窄判断，不要靠想象补齐
...
- 综合报告最前面也必须先写 `摘要概览` 与 `关键词`
- 在 `摘要概览`、`关键词` 后，必须新增“分析总结：作为转转广告投放专员，应该如何在小红书投广告”
- 这部分要根据关键结论给出可执行建议，例如面向什么用户、用什么内容格式、如何表达、用什么证据增强信任、怎样利用评论区辅助转化
- 综合报告中的关键主结论和关键子结论同样要补必要例证，并保留每个子结构的自由发挥空间
