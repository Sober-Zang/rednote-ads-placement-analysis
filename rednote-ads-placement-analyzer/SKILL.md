...
- 临时文件只允许写入当前官方 run 的 `temp/` 或系统临时目录。
...
- 标准任务的报告正文必须由模型基于当前 run 的证据动态生成；禁止把固定正文、临时脚本或仓库内草稿文件当作正式分析路径。
...
目标：先由官方脚本把当前 run 的标准提示词、样本证据、输出路径和质量要求编译成分析任务包，再由模型基于这些输入生成单篇报告与综合报告并落盘。
...
- 先读取 `prompt/compiled_analysis_prompt.md`、各样本 `manifests/evidence_digest.json` 与 `logs/report_quality_requirements.json`
- 生成单篇报告前，必须先读取当前样本目录下的 `manifests/note_manifest.json`；报告头部是否写入登录标签与风险提示，只能以该文件中的 `login_mode` 和 `login_mode_note` 为唯一事实来源
- 图片必须真实提供给模型阅读和思考，尤其是关键正文图片；未提供的图片不得假装已读
- 若本次是标准任务：
  ...
  - 报告必须恢复 `v1.1.2` 风格的中间推理层：互动数据、文本×评论、图片×评论、差异机制、风险与边界、可迁移性判断
- 若本次是自定义任务且同时包含小红书链接：
  - 先完成下载与归档
  - 再改用本次 run 的专属 Prompt 回答用户指定目标
  - 这类回答不必强制固化成 `final_broadcast.md`
- 若执行过程中确需临时文件，只能写到当前 run 的 `temp/`；严禁在仓库源码树内创建脚本、Markdown 草稿或中间产物

### 阶段 5：最终落档与契约校验

目标：在标准任务下完成最终播报与契约校验；没有报告就不能把标准任务 finalize 成完成态。
...
- 若校验发现仓库源码树内存在非法写回文件，必须视为执行违规而不是忽略
