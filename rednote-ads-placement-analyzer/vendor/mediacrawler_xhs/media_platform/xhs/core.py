...
class XiaoHongShuCrawler(AbstractCrawler):
    ...
    def __init__(self) -> None:
        ...
        self._vendor_root = Path(__file__).resolve().parents[2]
            ...
            if not await self.xhs_client.pong():
                if getattr(config, "REQUIRE_LOGIN", False):
                    ...
                    utils.logger.warning(
                        "[XiaoHongShuCrawler.start] Login state invalid, continue in anonymous fallback mode."
                    )
    ...
    async def get_comments(self, note_id: str, xsec_token: str, semaphore: asyncio.Semaphore):
        ...
        async with semaphore:
            ...
            try:
                ...
                utils.logger.error(
                    f"[XiaoHongShuCrawler.get_comments] Skip comments for note {note_id} because crawling failed: {ex}"
                )
                return
    ...
    async def close(self):
        ...
        if self.cdp_manager:
            ...
            try:
                ...
                utils.logger.warning(f"[XiaoHongShuCrawler.close] Browser context already closed: {ex}")
